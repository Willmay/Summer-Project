...
def comment(request, post_id):
    ...
    if request.method == 'POST':
        if comment_form.is_valid():
            comment = Comment(user=request.user, text=comment_form.cleaned_data['text'],
                              post=Post.objects.get(id=post_id), image=comment_form.cleaned_data['image'])
            comment.save()
        ...
        return render(request, "comments/thread.html",
                      context={"comments": comments, "post_id": post_id, "comment_form": comment_form})
